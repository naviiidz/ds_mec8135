...
static const int NUM_LMKS       = 6; //Navid: 5 changed to 6
static const int NUM_FACTORS    = 10; //Navid: 9 changed to 10
...
int main()
{
    ...
    vector<VectorB> landmarks(NUM_LMKS), landmarks_simu;
    {
        ...
        b3 << 3.0, -1.0;
        b4 << 3.0,  1.0;
        b5 << 0.0,  0.0; //Navid: assume GPS as a landmark at 0 (GLOBAL)
        ...
        landmarks_simu.push_back(b5); // Navid: adding landmark to the vector landmarks_simu

    } // destroy b0...b4

    // Define the beacon's measurements in R^2
    VectorY             y, y_noise;
    ArrayY              y_sigmas, gps_sigmas;
    MatrixY             R; // Covariance
    MatrixY             S; // sqrt Info
    vector<map<int,VectorY>> measurements(NUM_POSES); // y = measurements[pose_id][lmk_id]

    y_sigmas << 1.0, 1.0; // Navid: changed these values
    gps_sigmas << 0.0001, 0.0001; // Navid: Add gps_sigmas
    ...
    for (int i = 0; i < NUM_POSES; ++i)
    {
        ...
        {   
            if (k==5){ // Navid: Added if, else content was there
            // simulate measurement
                b       = landmarks_simu[k];              // lmk coordinates in world frame
                //gps_sigmas << 0.0001, 0.0001, 0.0001;
                y_noise = gps_sigmas * ArrayY::Random();      // measurement noise
