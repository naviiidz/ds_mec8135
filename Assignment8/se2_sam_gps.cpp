...
static const int NUM_MEAS       = NUM_POSES * DoF + NUM_FACTORS * Dim+3;
...
int main()
{
    ...
    MatrixT         J_d_xi, J_d_xj, J_a, J_b, J_c; // Jacobian of motion wrt poses i and j
    ...
    for (int iteration = 0; iteration < MAX_ITER; ++iteration)
    {
        ...
        d  = poses_simu[1].rminus(poses[1], J_d_xj, J_d_xi); // expected motion = Xj (-) Xi
        r.segment<DoF>(row)         += W_gps * d.coeffs(); // residual
        J.block<DoF, DoF>(row, DoF) = W_gps * J_d_xi;
        row += DoF;
        
        /*
        d  = poses_simu[1].rminus(poses[1], J_d_xj, J_a); // expected motion = Xj (-) Xi
        r.segment<DoF>(row)= W_gps * (d+poses[0].lminus(SE2d::Identity(), J_b)).coeffs();
        J.block<DoF, DoF>(row, DoF) = W_gps *J_a;        
        J.block<DoF, DoF>(row, 0) = W_gps *J_b;        
        row += DoF;
        */
